...
# Univariate Analysis of Deviation for GLMs

This tests whether each morphometric variable significantly improves the model's ability to predict sex. The purpose of this is to dplyr::select the most relevant variables for inclusion in the final model.

This compares two models: one with the variable and one without it. The deviance (a measure of model fit) is used to assess the importance of the variable. A significant reduction in deviance means the variable is important.

```{r}
# Univariate analysis for each variable in the logistic model
univariate_results <- lapply(names(petrel_sexing)[2:9], function(measurement) {
  formula <- as.formula(paste("sex ~", measurement))
  glm_model_univariate <- glm(formula, family = binomial(link = "logit"), data = petrel_sexing)
  summary(glm_model_univariate)
})
# View the univariate results
univariate_results
```